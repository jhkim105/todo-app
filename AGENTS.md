...
- **가독성 및 시각화 (Readability)**: 긴 줄글보다는 **비교 표(Table)**, **목록(List)**, **흐름도(Mermaid)**를 적극 활용하여 한눈에 파악하기 쉽게 작성합니다.
  - **🚨 Mermaid 문법 주의**: 노드 ID나 레이블 텍스트에 특수문자(슬래시 `/`, 백슬래시 `\`, 괄호 `()`, 대괄호 `[]`, 공백 등)가 포함될 경우, GitHub 등에서 파싱 에러(Unable to render rich display)가 발생할 수 있습니다. 에러 방지를 위해 이와 같은 텍스트는 반드시 큰따옴표로 감쌉니다. (예: `J2["/openapi.json"]`, `SW2["/docs Swagger UI"]`)
